...
vec rotate_vec(vec v, double rad) {return vec(v.x*cos(rad)-v.y*sin(rad), v.x*sin(rad)+v.y*cos(rad));}
...
double ConvexPolygonArea(const vector<point> &P)
{
    double area = 0;
...
int main()
{
    ...
    while (cnt--)
    {
        ...
        for (int i = 0; i < N; i++)
        {
            scanf("%lf%lf%lf%lf%lf", &x, &y, &w, &h, &q);
            point o = point(x,y);
            double ang = -q / 180 * PI;
            p1 = translate(o, rotate_vec(vec(-w/2, -h/2), ang));
            p2 = translate(o, rotate_vec(vec(w/2, -h/2), ang));
            p3 = translate(o, rotate_vec(vec(-w/2, h/2), ang));
            p4 = translate(o, rotate_vec(vec(w/2, h/2), ang));
            sarea += w*h;
            allp.push_back(p1);allp.push_back(p2);allp.push_back(p3);allp.push_back(p4);
        }
        allp = monotone_chain(allp);
        charea = area(allp);
        //charea = ConvexPolygonArea(allp);
        double ans = sarea * 100 / charea;
        printf("%.1lf %%\n",sarea * 100 / charea);
